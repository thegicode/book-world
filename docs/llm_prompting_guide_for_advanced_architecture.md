...
이 문서는 `FavoriteNav_260221.ts`에서 확립한 **최상위 레벨(Transcendent)** 아키텍처를 새로운 컴포넌트에 일관되게 적용하기 위한 LLM 프롬프트 전략을 담고 있습니다.
...
### 🔹 [Context] 참조 코드 제공
> "새로 만들 컴포넌트의 구조는 첨부된 `FavoriteNav_260221.ts`의 아키텍처를 모범 사례(Best Practice)로 삼아줘. 특히 `requestUpdate` 시스템과 `ModelController` 패턴을 그대로 유지해야 해."
...
**요구사항**:
1. **아키텍처**: `FavoriteNav_260221.ts`와 동일한 'Transcendent' 패턴을 사용하라.
...
---
*이 가이드를 활용하면 일관된 고품질의 코드를 유지하며 프로젝트를 확장할 수 있습니다.*
