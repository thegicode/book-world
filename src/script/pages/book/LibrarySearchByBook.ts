...
export default class LibrarySearchByBook extends HTMLElement {
    ...
    async fetchLibrarySearchByBook(isbn: string, region: string, dtl_region:string) {
    ...
    render({ libs }: LibrarySearchResult, isbn: string) {
        ...
        libs.forEach(({ homepage, libCode, libName }) => {
            const template = document.querySelector('#tp-librarySearchByBookItem') as HTMLTemplateElement
            if (!template) return
    
            const cloned = template.content.querySelector(':scope > *')!.cloneNode(true) as HTMLElement
    ...
    async fetchLoadnAvailabilty(isbn13: string, libCode: string): Promise<boolean> {
        const searchParams = new URLSearchParams({
            isbn13,
            libCode
        })
